...
namespace CV5
{
    public partial class frmVentasDiarias : Form
    {
        ...
        private async void btnOk_Click(object sender, EventArgs e)
        {
            ...
            txtValorTotal.Text = totalValorNeto.ToString();
            txtNC.Text = NC.ToString();
            txtValorBruto.Text = totalValorBruto.ToString();
            txtValorNeto.Text = totalValorNeto.ToString();

            //await Task.Run(() =>
            //{
            //ConexionMba cs = new ConexionMba();
            //    if (!string.IsNullOrEmpty(this.cmbLocalidad.Text))
            //    {
        ...
        private void cmbEmpresa_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string _CORP = "SELECT CORP FROM SIST_PARAMETROS_EMPRESA" +
                          " WHERE `CORPORATION NAM` = '" + cmbEmpresa.Text + "'";
            string query = "SELECT VEN.DESCRIPTION_SPN " +
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            float[] widths = new float[] {1f, 1f, 1f, 1f, 1.5f, 1f, 1f,
        ...
        void  ObtenerFacturas()
        {
            ...
            if (!fg.CheckDatePicker(dtpFechAct,dtpFechFin))
            {
                ...
                string cadena = " SELECT FP.EMPRESA `EMPRESA `, CASE FP.ORIGEN  WHEN 'PRI' THEN 'COSTA'" +
                    ...
                    // " FP.codigo_cliente, " +
                    " DATE_TO_CHAR(FP.fecha_factura, 'dd[/]mm[/]yyyy') AS `FECHA FACT`, " +
                    " CAST(FP.VALOR_FACTURA AS float) `VALOR TOTAL`, SUBSTRING(FP.codigo_factura,1,17) AS `FACTURA`, " +
